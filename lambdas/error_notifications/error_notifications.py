...
class StateMachineError(Error):
    def __init__(self, event) -> None:
        error = event["Error"]
        execution_id = event["ExecutionId"]
        statemachine_name = event["StateMachineName"]
        execution_details_url = self.__get_state_machine_execution_url(execution_id)
        try:
    ...
    def __glue_job_failure_message(self, statemachine_name, error, execution_details_url):
    ...
    def __generic_failure_message(self, statemachine_name, error, execution_details_url):
        message = (
    ...
    def __get_state_machine_execution_url(self, execution_id):
...
def main(event, _, sns_client=None, sf_client=None):
    ...
    if "StateMachineName" in event:
        error = StateMachineError(event)
    else:
        error = Error(event)

    try:
        client = Email(sns_client)
