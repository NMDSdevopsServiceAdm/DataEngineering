...
`copy-main-data` syncs `sfc-main-datasets`' `domain=sample_archive_data` job
role archive datasets into every branch's own dataset bucket (under the
renamed `domain=03_ind_cqc` path) on its first deploy. This narrows that
