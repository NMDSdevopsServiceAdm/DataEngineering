...
class TestNullNotKnownValues:
    @pytest.mark.parametrize(
        "case",
        [
            pytest.param(case, id=case.id)
            for case in Data.null_not_known_values_test_cases
        ],
    )
    def test_null_not_known_values(self, case):
        test_lf = pl.LazyFrame(case.test_data, case.test_schema, orient="row")

        returned_lf = job.null_not_known_values(
            test_lf,
            columns_to_clean=case.columns_to_clean,
            not_known_code=case.not_known_code,
            populated_rule=case.populated_rule,
            missing_rule=case.missing_rule,
            not_known_rule=case.not_known_rule,
        )
