...
class CalculateWindowedColumnTests(unittest.TestCase):
    ...
    def test_calculate_windowed_column_with_agg_function_avg(self):
        returned_lf = utils.calculate_windowed_column(
            ...
            input_column_name=IndCQC.ascwds_filled_posts,
            aggregation_function="avg",
            partition_by_cols=IndCQC.care_home,
        )
        expected_lf = pl.LazyFrame(
            Data.expected_calculate_windowed_column_avg_rows,
            Schemas.expected_schema_with_float_column,
            orient="row",
        )
    ...
    def test_calculate_windowed_column_count(self):
        ...
        expected_lf = pl.LazyFrame(
            ...
            Schemas.expected_schema_with_int_column,
            orient="row",
        )
        returned_data = returned_lf.sort(IndCQC.location_id)
        expected_data = expected_lf
