...
class MainTests(EstimateIndCQCFilledPostsByJobRoleTests):
    ...
    @patch(
        "utils.ind_cqc_filled_posts_by_job_role_utils.utils.count_job_role_per_establishment_as_columns"
    )
    @patch(
        "utils.ind_cqc_filled_posts_by_job_role_utils.utils.count_registered_manager_names"
    )
