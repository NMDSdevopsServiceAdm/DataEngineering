...
class JoinCQCLocationDataIntoASCWDSWorkplaceDataframe(ReconciliationTests):
    def setUp(self) -> None:
        ...
        self.returned_df = job.join_cqc_location_data_into_ascwds_workplace_df(self.test_ascwds_df, self.test_cqc_df)
        self.expected_df.show()
        self.returned_df.show()


    def test_join_cqc_locations_data_into_ascwds_workplace_df_returns_correct_values(self):
        returned_data = self.returned_df.collect()
        expected_data = self.expected_df.collect()
        self.assertEqual(
            returned_data, expected_data
