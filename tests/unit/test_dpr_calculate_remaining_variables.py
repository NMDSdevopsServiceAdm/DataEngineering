...
class TestCalculateRemainingVariables(unittest.TestCase):
    ...
    def test_calculate_total_dpr_employing_staff_returns_correct_values(self):
        rows = [
            ("area_1", 2021, 0.49, 400.0, 13.82054836444813, 0.191616868609776),
            ("area_2", 2021, 0.34, 390.0, 14.68208904171243, 0.2043913265170944),
        ]
        test_schema = StructType(
            [
                StructField(DP.LA_AREA, StringType(), False),
        ...
        output_df_list = output_df.sort(DP.LA_AREA).collect()
        self.assertAlmostEqual(output_df_list[0][DP.ESTIMATED_TOTAL_DPR_EMPLOYING_STAFF], 414.0121652330579, places=5)
        self.assertAlmostEqual(output_df_list[1][DP.ESTIMATED_TOTAL_DPR_EMPLOYING_STAFF], 404.8864803682295, places=5)
