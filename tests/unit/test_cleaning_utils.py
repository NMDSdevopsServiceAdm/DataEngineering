...
class TruncatePostcodeTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.test_df = self.spark.createDataFrame(
            Data.truncate_postcode_rows,
            Schemas.truncate_postcode_schema,
        )
        self.returned_df = job.truncate_postcode(
            self.test_df,
            CQCLClean.postcode,
            drop_postcode_col=False,
        )
    ...
    def test_truncate_postcode_adds_new_column(self):
        self.assertIn(CQCLClean.postcode_truncated, self.returned_df.columns)

    def test_truncate_postcode_does_not_drop_postcode_col_when_drop_postcode_col_is_false(
        self,
    ):
        self.assertIn(CQCLClean.postcode, self.returned_df.columns)

    def test_truncate_postcode_drops_postcode_col_when_drop_postcode_col_is_true(self):
        returned_df = job.truncate_postcode(
            self.test_df,
            CQCLClean.postcode,
            drop_postcode_col=True,
        )

        self.assertNotIn(CQCLClean.postcode, returned_df.columns)
