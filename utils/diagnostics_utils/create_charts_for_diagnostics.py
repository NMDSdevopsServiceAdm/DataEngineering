...
def create_charts_for_care_home_model_diagnostics(
    ...
    care_home_model_df = filter_to_care_home_model(df)

    with io.BytesIO() as output:
        with PdfPages(output) as pp:
            for chart in list_of_charts:
                fig = create_figure(care_home_model_df, chart)
                pp.savefig(fig)
...
def filter_to_care_home_model(df: DataFrame) -> DataFrame:
    care_home_model_df = df.where(
        (
