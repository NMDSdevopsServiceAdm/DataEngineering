...
def determine_areas_including_carers_on_adass(
    ...
    most_recent_direct_payments_df = calculate_total_dprs_during_year(
...
def calculate_total_dprs_during_year(df: DataFrame) -> DataFrame:
    df = df.withColumn(
        ...
        df[DP.SERVICE_USER_DPRS_DURING_YEAR] + df[DP.CARER_DPRS_DURING_YEAR],
    )
    return df
...
def calculate_service_users_employing_staff(df: DataFrame) -> DataFrame:
    df = df.withColumn(
        ...
        df[DP.SERVICE_USER_DPRS_DURING_YEAR]
        * df[DP.PROPORTION_OF_SERVICE_USERS_EMPLOYING_STAFF],
    )
    return df
...
def calculate_difference_between_survey_base_and_total_dpr_during_year(
    df: DataFrame,
) -> DataFrame:
    df = df.withColumn(
