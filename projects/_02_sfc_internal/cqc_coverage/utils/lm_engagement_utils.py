...
def add_columns_for_locality_manager_dashboard(df: DataFrame) -> DataFrame:
    ...
    """
    df = df.withColumn(year_col, F.year(F.col(CQCLClean.cqc_location_import_date)))
...
def create_windows_for_lm_engagement_calculations() -> Tuple[Window, Window, Window]:
    ...
    Returns:
        Tuple[Window, Window, Window]: A tuple of windows required for the
            locality manager dashboard.
    """
    ...
    ytd_w = (
        Window.partitionBy(CQCLClean.current_cssr, year_col)
        .orderBy(CQCLClean.cqc_location_import_date)
...
def calculate_la_coverage_monthly(df: DataFrame, agg_w: Window) -> DataFrame:
    ...
    Args:
        df (DataFrame): A dataframe with coverage data
        agg_w (Window): A window which aggregates the data over local
            authorities and import dates

    Returns:
        DataFrame: The same dataframe with the monthly coverage column added.
    """
    la_monthly_locations_count = F.count(F.col(CQCLClean.location_id)).over(agg_w)
    la_monthly_locations_in_ascwds_count = F.sum(F.col(CoverageColumns.in_ascwds)).over(
        agg_w
    )
...
def calculate_coverage_monthly_change(df: DataFrame, w: Window) -> DataFrame:
    ...
    Returns:
        DataFrame: The same dataframe with the coverage monthly change column
        added.
    """
    la_monthly_coverage_last_month = F.lag(CoverageColumns.la_monthly_coverage).over(w)

    return df.withColumn(
        CoverageColumns.coverage_monthly_change,
        F.col(CoverageColumns.la_monthly_coverage) - la_monthly_coverage_last_month,
    )
...
def calculate_locations_monthly_change(
    ...
    """
    Adds a column with the locations monthly change for the local authority and
    in ascwds last month column for the location.
...
def calculate_new_registrations(
    ...
    """
    Adds columns with the monthly new registrations and the number of new
    registrations for the year to date for the local authority.

    Adds columns with the monthly new registrations and the number of new
    registrations for the year to date for the local authority. The in ascwds
    last month column is also dropped after being used for these calculations.
    NB: new registrations do not take into account new de-registrations in their
    calculation.
    ...
    Args:
        ...
        agg_w (Window): A window which aggregates the data over local
            authorities and import dates
        ytd_w (Window): A window which aggregates the year to date for local
            authorities and years

    Returns:
        DataFrame: The same dataframe with with the monthly new registrations
        and the number of new registrations for the year to date columns added.
    """
    new_registration = F.when(
        (
    ...
    return df.withColumns(
        {
            CoverageColumns.new_registrations_monthly: F.sum(new_registration).over(
                agg_w
            ),
            CoverageColumns.new_registrations_ytd: F.sum(new_registration).over(ytd_w),
        }
