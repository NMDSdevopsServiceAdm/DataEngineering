...
QUERY_PLAN_GRAPH_PATH = "/tmp/candidate_a_query_plan.png"
...
def main(
    ...
    Candidate A's four-unpivot-plus-join approach. Logs run time, peak RSS and
    the streaming query plan around the reshape and sink steps, to compare
    against Candidate B's prototype.
    ...
    """
    print(f"[prepare][A] peak RSS before scan: {prepare_utils.peak_rss_kb()} KB")
    ...
    print(f"[prepare][A] reshape build time (lazy): {reshape_elapsed:.3f}s")
    print(
        f"[prepare][A] peak RSS after reshape built: {prepare_utils.peak_rss_kb()} KB"
    )
    print(f"[prepare][A] query plan:\n{job_roles_lf.explain(engine='streaming')}")
    ...
    try:
        ...
        print(f"[prepare][A] could not render query plan graph: {graph_error}")
    ...
    print(f"[prepare][A] sink time: {sink_elapsed:.3f}s")
    print(f"[prepare][A] peak RSS after sink: {prepare_utils.peak_rss_kb()} KB")
