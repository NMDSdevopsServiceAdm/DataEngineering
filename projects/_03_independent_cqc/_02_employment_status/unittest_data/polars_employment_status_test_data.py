    ...
    null_counts_for_low_location_ratio_test_cases = [
        CleanUtilsTestCase(
            ...
            input_data={
                ...
                IndCQC.ascwds_worker_import_date: [CLEAN_UTILS_IMPORT_DATE] * 5,
                # "other" fills each row's raw total up to the intended
                # per-location staff figure, since staff is the 5 raw
                ...
                EmpStatus.other_count: [10, 10, 9, 10, 5],
    ...
    null_counts_for_low_org_ratio_test_cases = [
        CleanUtilsTestCase(
            ...
            input_data={
                ...
                IndCQC.ascwds_worker_import_date: [CLEAN_UTILS_IMPORT_DATE] * 2,
        ...
        CleanUtilsTestCase(
            ...
            input_data={
                # Each row's raw total is already row-specific (unlike a
                # worker_records_bounded-based denominator, which repeats
                # per location and would need an extra distinct-location
                # step to avoid inflating with job-role row count) -
                # summing every row's raw total directly, across both
                # locations, is correct with no such step.
                IndCQC.organisation_id: ["org2"] * 4,
                IndCQC.location_id: ["loc1", "loc1", "loc2", "loc2"],
                IndCQC.establishment_id: ["est1", "est1", "est2", "est2"],
                IndCQC.ascwds_worker_import_date: [CLEAN_UTILS_IMPORT_DATE] * 4,
        ...
        CleanUtilsTestCase(
            ...
            input_data={
                # loc1's 2nd job role has no worker records (raw counts
                # never recorded), but its 1st job role alone keeps the
                # org ratio fine - the null row must not default to
                # "populated". The null row also drops out of
                # org_total_staff entirely (its raw total is null), rather
                # than counting as unstaffed.
