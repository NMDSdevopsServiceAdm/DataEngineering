...
def assign_location_folds(
    ...
    location_folds_lf = (
        ...
        .select(
            ...
            (pl.int_range(pl.len()) % n_folds).cast(pl.UInt8).alias(ShareModel.fold),
        )
    )
...
def add_fold_safe_rolling_average(
    ...
    For each fold, that fold's input shares are blanked, `rolling_average_function` is run, and
    only that fold's rows are kept. So a tested fold's rows still get their group's average, even
    rows with no share of their own, but their own shares don't feed into it. The kept outputs
    are joined back onto `lf`, so every row keeps its own input shares and the row count doesn't
    ...
    Args:
        ...
        input_columns (list[str]): the shares the rolling average is made from
