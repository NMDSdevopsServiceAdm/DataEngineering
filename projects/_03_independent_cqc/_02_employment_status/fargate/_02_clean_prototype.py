...
Wraps the .over()-based EmpStat ratio-filter clean job with RunDiagnostics
to measure peak memory, for comparison against the aggregate-then-join-back
variant's equivalent prototype on the 2094-join-spike branch. Delete this
file (and its terraform/Dockerfile/Step Function wiring) once that
...
def main(merged_data_source: str, cleaned_data_destination: str) -> None:
    """
    Runs the .over()-based EmpStat clean job under RunDiagnostics.
    ...
    diagnostics = RunDiagnostics("empstat_02_clean_over", data_bucket).start()
