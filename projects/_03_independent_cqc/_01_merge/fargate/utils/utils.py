...
def aggregate_ascwds_worker_job_roles_per_establishment(
    ...
    to get a job role count per workplace per import date. A row is added for all potential job roles per workplace per import date,
    and if a workplace does not have any of a particular role then the count is 0.
    ...
    Args:
        ...
        columns_to_aggregate_on (list): A list of columns to group_by. Columns not in this list will be dropped.

    Returns:
        pl.LazyFrame: The input LazyFrame with a new column for the count job roles per workplace.
    """
    worker_count_lf = lf.group_by(columns_to_aggregate_on).len(
    ...
    worker_count_lf = worker_count_lf.group_by(columns_to_remain_as_rows).agg(
        aggregation_per_job_role
    )
    ...
    worker_count_lf = worker_count_lf.unpivot(
        index=group_identification_columns,
