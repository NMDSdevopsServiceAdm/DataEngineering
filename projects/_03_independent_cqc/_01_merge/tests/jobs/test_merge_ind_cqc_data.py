...
class MergeIndCQCDatasetTests(unittest.TestCase):
    ...
    @patch(f"{PATCH_PATH}.remove_pir_data")
    @patch(f"{PATCH_PATH}.remove_july_2025_cqc")
    @patch(f"{PATCH_PATH}.utils.read_from_parquet")
    def test_main_runs(
        self,
        read_from_parquet_patch: Mock,
        remove_july_2025_cqc_mock: Mock,
        remove_pir_data_mock: Mock,
    ...
    def test_remove_pir_data(
        self,
    ):
        test_df = self.spark.createDataFrame(
            Data.remove_may_2025_rows, Schemas.remove_may_2025_schema
        )

        returned_df = job.remove_pir_data(test_df)

        expected_df = self.spark.createDataFrame(
            Data.expected_remove_may_2025_rows,
            Schemas.remove_may_2025_schema,
        )
