...
job_role_codes_and_labels: dict[str, str] = MAIN_JOB_ROLE.code_to_label()
all_zero_filled_job_role_codes_and_labels: dict[str, str] = {
    code.zfill(2): label for code, label in job_role_codes_and_labels.items()
}
...
for _code, _label in all_zero_filled_job_role_codes_and_labels.items():
    ...
    if _label not in AscwdsWorkerValueLabelsJobGroup.job_role_to_job_group_dict:
        raise KeyError(
            ...
            "MAIN_JOB_ROLE.code_to_label() but has no entry in "
...
def reduce_to_published_roles(lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    other catalogued raw job role (MAIN_JOB_ROLE) is summed into whichever of
    the 4 'other_*' synthetic roles (jr1001/jr1002/jr1003/jr1004) its job
    group (AscwdsWorkerValueLabelsJobGroup) maps to, then dropped. A sum is
    null only when all of its contributing columns are null.
    ...
    Raises:
        ValueError: If a job role column's code isn't in
            MAIN_JOB_ROLE.code_to_label() (an ASC-WDS code this team hasn't
            catalogued yet).
    """
    job_role_matches = {
    ...
    if unknown_codes:
        raise ValueError(
            ...
            "input columns. Add them to the unified main_job_role vocabulary "
            "(MAIN_JOB_ROLE) and classify them (either as a new published role in "
            "PublishedJobRoleLabels, or via AscwdsWorkerValueLabelsJobGroup) before "
            "running reduce_to_published_roles."
        )
...
def reshape_job_role_cols_to_rows(lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    columns (establishment_id, ascwds_workplace_import_date,
    published_job_role_label), establishment_id and published_job_role_label
    are Categorical/Enum rather than String to keep downstream uniqueness
    checks cheap at this dataset's scale.
...
def relabel_job_role_columns(lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    codes are looked up in the unified main_job_role vocabulary
    (MAIN_JOB_ROLE); the four synthetic codes produced by
    reduce_to_published_roles are looked up in SYNTHETIC_JOB_ROLE_LABELS
    instead. Columns not matching the jrNN{suffix} shape are left untouched.
    ...
    for col in lf.collect_schema().names():
        ...
        if code in SYNTHETIC_JOB_ROLE_LABELS:
            ...
            if stripped_code not in job_role_codes_and_labels:
                raise ValueError(f"Unmapped job role code: '{code}'")
            label = job_role_codes_and_labels[stripped_code]
