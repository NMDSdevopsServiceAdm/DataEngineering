...
def model_interpolation(
    ...
    Returns:
        pl.LazyFrame: The LazyFrame with the interpolated values in the 'interpolation_model' column.
    ...
    if method == "trend":
        ...
        lf.sort([IndCqc.location_id, IndCqc.unix_time])
        lf = lf.with_columns(
            (
...
def calculate_residuals(
    ...
    Returns:
        pl.LazyFrame: The LazyFrame with the calculated residuals in a new column.
    """
    lf = lf.sort([IndCqc.location_id, IndCqc.unix_time])
...
def calculate_proportion_of_time_between_submissions(
    ...
    lf = (
        lf.sort([IndCqc.location_id, IndCqc.unix_time])
        .with_columns(
            # Create conditional column: unix_time where column_with_null_values is not null
            pl.when(pl.col(column_with_null_values).is_not_null())
            .then(pl.col(IndCqc.unix_time))
            .alias("_temp_time")
        )
        .with_columns(
    ...
    current_unix_time_is_between_known_submissions = (
        pl.col(IndCqc.previous_submission_time) < pl.col(IndCqc.unix_time)
    ) & (pl.col(IndCqc.next_submission_time) > pl.col(IndCqc.unix_time))

    lf = lf.with_columns(
        pl.when(current_unix_time_is_between_known_submissions)
        .then(
    ...
    lf = lf.with_columns(
        pl.when(current_unix_time_is_between_known_submissions)
        .then(
            (pl.col(IndCqc.unix_time) - pl.col(IndCqc.previous_submission_time))
...
def calculate_interpolated_values(
    ...
    """
    Calculate interpolated values for a new column in a LazyFrame, optionally constrained by a max time between submissions.

    This function takes a LazyFrame and interpolates values from an existing column to create a new column.
    ...
    Returns:
        pl.LazyFrame: A new LazyFrame with the interpolated values added as a new column.
    """
    if max_days_between_submissions is not None:
        max_time = convert_days_to_unix_time(max_days_between_submissions)
        condition_is_true = pl.col(IndCqc.time_between_submissions) <= pl.lit(max_time)
    else:
