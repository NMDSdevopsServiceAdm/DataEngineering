...
from projects._03_independent_cqc._06_estimate_filled_posts.fargate.utils.models.extrapolation import (
    model_extrapolation,
)
from projects._03_independent_cqc._06_estimate_filled_posts.fargate.utils.models.interpolation import (
    model_interpolation,
)
...
def model_imputation_with_extrapolation_and_interpolation(
    ...
    Args:
        lf (pl.LazyFrame): The input LazyFrame containing the columns to be extrapolated and interpolated.
    ...
    """
    # identify locations with a non null submission
    lf = lf.with_columns(
        (
        ...
        ).alias(IndCqc.has_non_null_value)
    )

    # split datasets for imputations
    if care_home == True:
        care_home_filter_value: str = CareHome.care_home
    else:
        care_home_filter_value: str = CareHome.not_care_home

    imputation_lf = lf.filter(
        (pl.col(IndCqc.care_home) == care_home_filter_value)
        & (pl.col(IndCqc.has_non_null_value) == True)
    )
    non_imputation_lf = lf.filter(
        (pl.col(IndCqc.care_home) != care_home_filter_value)
        | (pl.col(IndCqc.has_non_null_value) == False)
    )
