...
Function wiring, and clean_ascwds_workplace_baseline_prototype.py once the
investigation concludes.

Copies clean_ascwds_workplace.py::main() as of this branch, instrumented
with RunDiagnostics, and lets the `FIX_VARIANT` env var (A/B/C) select how
`duplicate_keys` is computed - see the plan for what each variant changes.
"""
...
FIX_VARIANT = os.environ.get("FIX_VARIANT", "A")
...
def compute_duplicate_keys(
    combined_lf: pl.LazyFrame, diagnostics: RunDiagnostics
) -> pl.LazyFrame:
    """Computes duplicate_keys per FIX_VARIANT - see module docstring."""
    diagnostics.checkpoint("before_duplicate_detection", combined_lf)
    ...
    if FIX_VARIANT == "A":
        ...
        import_dates = (
            combined_lf.select(AWPClean.import_date)
            .unique()
            .collect()
            .to_series()
            .to_list()
        ...
        partial_results = [
            wUtils.find_duplicate_workplace_submissions(
                combined_lf.filter(pl.col(AWPClean.import_date) == import_date)
            ).collect()
        ...
        duplicate_keys = (
            ...
            if partial_results
            else wUtils.find_duplicate_workplace_submissions(combined_lf).collect()
        ).lazy()
    else:
        raise ValueError(f"Unknown FIX_VARIANT: {FIX_VARIANT!r}")

    diagnostics.checkpoint("after_duplicate_detection", duplicate_keys)
...
def main(
    ...
    """Clean raw ASC-WDS data - throwaway diagnostics copy, see module docstring."""
    data_bucket, _ = split_s3_uri(cleaned_workplace_destination)
    diagnostics = RunDiagnostics(
        f"ascwds_workplace_prototype_{FIX_VARIANT}", data_bucket
    ).start()
    ...
    try:
        ...
        duplicate_keys = compute_duplicate_keys(combined_lf, diagnostics)
        ...
        workplace_lf = wUtils.null_duplicate_workplace_data(
            workplace_lf, duplicate_keys
        )
