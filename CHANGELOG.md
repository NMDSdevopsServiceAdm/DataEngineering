...
- Narrowed dtypes (Categorical/Enum/Float32) for the columns feeding the independent CQC estimate archive jobs, to reduce long-term storage and read cost: ASCWDS source/filtering-rule labels and the job-role ratio merge source are now Enum, and several model/estimate columns (including `estimate_filled_posts` and the job-role reallocation/reconciliation columns) are now Float32 instead of Float64, each cast at the column's true point of creation in the pipeline rather than downstream. Fixed a real dtype bug found along the way, where an unsigned-int registered-manager count was silently promoting `estimate_filled_posts_by_job_role_manager_adjusted` to Float64 via Polars' type coercion rules. Simplified the job-role reconciliation check to round the difference to 2dp and check it falls between 0 and 1, replacing the fixed absolute tolerance it previously used.

- Added a `col_schema_match` check to the independent CQC clean and estimate validation jobs (`validate_cleaned_ind_cqc_data.py`, `validate_estimated_ind_cqc_filled_posts_data.py`), which previously had no schema assertion at all, so a future dtype regression on those narrowed columns is caught at the earliest validation step rather than downstream. `current_cssr`, `current_region`, and `ascwds_filled_posts_source` now correctly expect Categorical/Enum in that check, now that the estimate stage reads the Polars impute output directly instead of the removed PySpark one.
