...
### Added
- Added validation checks for columns that are created but never checked: the Independent CQC filled posts clean job's `care_home_status_count` and 6 sibling columns; the CQC locations ingest chain's postcode-matched ONS geography columns, `relationships_types`, `postcode_cleaned` and `postal_code`; Capacity Tracker's `care_home` column; CQC PIR's `pir_submission_date_as_date`; the CQC providers ingest chain's `year`/`month`/`day` partition columns; the ONS postcode directory's own 18 geography columns at their source; 8 previously-uncovered columns in the workplace estimates job (model run IDs, imputed/combined model predictions, and the Capacity Tracker non-res estimate and its source); the impute job's banded-bed-count rolling-average columns; 10 previously-uncovered columns across the starters/leavers/vacancies prepare and clean stages; 8 previously-uncovered columns across the employment status prepare, merge and estimate stages; and 13 previously-uncovered columns in the direct payment recipients estimate job, which had no coverage of its actual output at all. Loosened the ind_cqc filled posts clean job's `ct_care_home_posts_per_bed_ratio` check to non-negative only, after its assumed 0-20 upper bound (mirrored from the similar ASCWDS-derived ratio) failed against real Capacity Tracker data. Dropped the same kind of unverified 0-1/-100-3000 bound from the workplace estimates job's `non_res_combined_model` (replaced with a not-null check scoped to non-care-home rows, since it's null by design for care homes) and the direct payments job's `ESTIMATED_PROPORTION_OF_SERVICE_USERS_EMPLOYING_STAFF` and its rolling average (both can inherit an unbounded value via extrapolation). Corrected `first_year_with_data`/`last_year_with_data`'s lower bound from a hardcoded 2013 (mirrored from an unrelated CQC/ASCWDS check) to `Config.FIRST_YEAR` (2011), the direct payments dataset's own documented earliest year.
...
### Fixed
- Fixed `write_reports` to log which validation step failed before attempting to build the HTML tabular report, since that report can itself raise (pointblank can't serialise a failing extract that includes a List/Struct column to CSV) and was masking which check actually failed.
