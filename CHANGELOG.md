...
### Changed
- Consolidated the ASC-WDS job-role, employment-status, establishment-type, parent-permission, is-parent, main-service and registration-type code↔label vocabulary (7 columns, corrected from the original 2-column scope) into unified Python structures in `utils/column_values/ascwds_labelled_vocab.py`, retiring `data_labels_lookup.csv` and the duplicate `ascwds_worker_mainjrid.py` job-role dict entirely. Documented the `pl.Enum` vs `pl.Categorical` rule on `CategoricalColumnTypes` and added the previously-missing `parent_permission` cast. Left the remaining `value_labels/` mechanisms (job-group derivation, CQC service-type mapping, ONS geography relabelling, ASC-WDS reconciliation) untouched as known related debt for a follow-up ticket.
