...
### Changed
- Replaced `docker login` with the AWS ECR credential helper in the `task-containerisation` CircleCI job, so the ECR auth token is no longer written to the job container's disk unencrypted.
