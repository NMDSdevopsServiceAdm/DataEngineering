...
- Added utilities for investigating models to estimate employment status percentage breakdowns: a modelling dataset builder and its features, location-grouped cross-validation folds, a fold-safe version of the rolling average baseline, and worker-weighted scoring and jumpiness measures. The employment status clean job also now copies its percentages into stand-in `_percentage_clean` columns until the cleaning filters are added.
