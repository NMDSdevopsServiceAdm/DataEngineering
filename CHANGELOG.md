...
### Changed
- Added a Polars version of the Flatten CQC Ratings job alongside the existing PySpark/Glue one (folder renumbered to `_01_cqc_ratings`), fixing a non-deterministic tiebreak in the assessment ratings pivot (now resolved by raw explode order); the two versions will run side by side for output comparison before the old Glue job and PySpark code are cut over and removed.
