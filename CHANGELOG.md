...
### Changed
- Renamed S3 dataset domains to a project-numbered scheme (`01_ascwds`, `01_cqc`, `01_ons`, `01_capacity_tracker`, `02_sfc`, `03_ind_cqc`, `04_dpr`, `99_publication`) across every Glue job, Step Function, EventBridge trigger, and the independent CQC model's path helpers, consolidating the workforce characteristics and sample archive data crawlers and Fargate task into the independent CQC crawler and task (renamed `ind_cqc_crawler`, now covering the whole independent CQC domain rather than just filled posts) and set `table_prefix` on the Glue crawler module so every crawler prefixes its discovered Athena table names with its domain. Removed the separate `data_validation_reports` domain and crawler entirely — validation reports now live alongside the dataset they validate with a `_validation`-suffixed dataset name, so they're picked up by that dataset's own domain crawler. Within `03_ind_cqc`, renumbered the filled posts pipeline's stages to close the gap left by a previously-removed stage 5 (`01_filled_posts_01`-`08`) and moved SLV under `03_starters_leavers_vacancies`, reserving `02_employment_status` for a product that doesn't exist yet. The job role archive now shares its stage number with the filled posts archive it will eventually replace, renamed to `01_filled_posts_08_sample_archive` to match PR 2018's naming. Removed the now-dangling `jr_archive_geography_source` argument from the publication merge job, since the archive job stopped producing a geography output. Dropped the redundant `cqc_`/`capacity_tracker_` prefix from CQC and capacity tracker dataset names now that their domain already carries it (e.g. `cqc_locations_04_full_cleaned_registered` → `locations_04_full_cleaned_registered`, `capacity_tracker_care_home` → `care_home`). Updated the two CQC delta-download jobs' `generate_s3_dir()` calls, CircleCI's branch-preview dataset sync and raw-bucket sample seed, and remaining test fixtures to the new paths throughout. The raw bucket's own upload prefixes (`domain=ASCWDS`/`CQC`/`ONS`/`capacity_tracker`, where the ASC-WDS/CQC PIR/ONS/Capacity Tracker CSVs actually land) are excluded from this renaming — they're hand-managed outside this repo and can't be changed here, so EventBridge's raw-bucket triggers and CircleCI's raw-bucket sample seed keep the old names; only the converted-to-parquet output in the datasets bucket picks up the new numbered domains. Documented the new naming scheme, the validation-suffix convention, and the raw-bucket exception in STYLEGUIDE.md. Trimmed redundant repetition from the filled posts stage names now that the domain and product prefix already say what they are (`01_filled_posts_01_merged_data` → `01_filled_posts_01_merged`, `_02_cleaned_data` → `_02_cleaned`, `_03_imputed_ascwds_and_pir` → `_03_imputed`, `_05_estimated_filled_posts` → `_05_estimated`, `_07_estimated_filled_posts_diagnostics` → `_07_estimated_diagnostics`). Restored CircleCI's dataset sync to `main`'s full list (rather than the trimmed raw-only set), with destination paths updated to the new naming, including the `main_`-prefixed compare-path datasets `step-function.tf`'s locals expect on non-main workspaces. Once `sfc-main-datasets` itself was renamed, updated the sync step's source paths to the new domains too (both sides now match except for the destination's `main_` prefix and bucket), including the archive sample seed, which now reads from `domain=03_ind_cqc` instead of the legacy `domain=sample_archive_data`. Fixed `construct_destination_path` (`utils/file_utils.py`, shared by all four raw ingestion jobs) to rename the raw file's fixed-domain key (e.g. `domain=ASCWDS`) to the numbered datasets-bucket domain (`domain=01_ascwds`) on the way through, since the raw bucket's domain names can't change but its own converted-to-parquet output needs to land under the new scheme; also corrected the corresponding test fixtures back to raw-domain source keys, which an earlier pass in this same ticket had wrongly renamed.
