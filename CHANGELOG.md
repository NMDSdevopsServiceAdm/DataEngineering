...
- Cast `care_home`, `dormancy`, `cqc_sector`, `primary_service_type`, `primary_service_type_second_level`, `current_rural_urban_ind_11` and the contemporary/current geography columns (region, CSSR, sub ICB, ICB, ICB region) to `Categorical`/`Enum` in the IND CQC merge job, so every downstream IND CQC pipeline stage inherits the narrower dtype regardless of which engine cleaned the original source. Added the corresponding named dtype constants to `polars_utils/column_types.py`, sharing one dtype across current/contemporary column pairs for region, CSSR and ICB (current geographies are always a subset of contemporary ones, named `Ons{Geography}CatType` to distinguish from other future non-ONS sources of the same geography) and using `Categorical` rather than `Enum` for job role/group columns and other categories expected to grow over time. Fixed two hardcoded `pl.Enum` casts of the job role column in the Estimate Filled Posts by Job Role pipeline that had fallen out of step with this change and were failing to join against the new `Categorical` dtype.
