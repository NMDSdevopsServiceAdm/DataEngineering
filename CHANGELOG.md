...
- Added utilities for investigating models to estimate employment status percentage breakdowns: a modelling dataset builder and its features, and worker-weighted scoring of predicted shares. Location-grouped cross-validation folds, a fold-safe version of the rolling average baseline and a period-to-period jumpiness measure are in a new `_03_independent_cqc` project-level module, so the filled posts model can use them too. The employment status clean job also now copies its percentages into stand-in `_percentage_clean` columns until the cleaning filters are added.
