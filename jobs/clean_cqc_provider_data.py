...
def get_cqc_provider_df(cqc_provider_source) -> pyspark.sql.DataFrame:
    spark = utils.get_spark()

    print(f"Reading CQC providers parquet from {cqc_provider_source}")
    cqc_provider_df = (
        spark.read.option("basePath", cqc_provider_source) #used for partitioning
...
def main(cqc_source: str, cleaned_cqc_destination: str):
    cqc_provider_df = get_cqc_provider_df(cqc_source)
    cleaned_cqc_provider_df = clean_cqc_provider_df(cqc_provider_df)
    write_cleaned_provider_df_to_parquet(cleaned_cqc_provider_df, cleaned_cqc_destination)
...
#Where we tell Glue how to run the file, and what to print out
    ...
    source, destination = utils.collect_arguments(
        ("--cqc_provider_source", "Source s3 directory for parquet CQC providers dataset"),
        ("--cqc_provider_cleaned", "Destination s3 directory for cleaned parquet CQC providers dataset"),
    )
