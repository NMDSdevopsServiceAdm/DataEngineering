...
def main(worker_source, ascwds_import_date, destination):
    spark = utils.get_spark()
    print("Estimating ethnicity")

    ethniUDF = udf(lambda x: get_keys_from_value(ETHNICITY_DICT, x), StringType())
    jobRoleUDF = udf(lambda x: get_keys_from_value(MAINJRID_DICT, x), StringType())
