...
def main(
    ...
    for snapshot_date_row in date_matrix:
        ...
        ascwds_workplace_df = purge_workplaces(
            ascwds_workplace_df
        )  # Remove workplaces that are out of date
...
def add_ascwds_workplace_structure(df):
    df = df.withColumn(
        ...
        F.when(df.isparent == "1", "Parent")
        # Subsidiary is not a parent but has a parent ID
        .when(df.isparent == "0" & df.parentid.isNotNull(), "Subsidiary")
...
def generate_closest_date_matrix(
    dataset_workplace, dataset_locations_api, dataset_providers_api
):
...
def purge_workplaces(input_df):
    ...
    input_df.drop(
        "isparent", "mupddate", "lastloggedin", "max_mupddate_and_lastloggedin"
    )
