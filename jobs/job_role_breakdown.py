...
def calculate_job_count_breakdown_by_jobrole(master_df):
    ...
    master_df = master_df.withColumn("adjusted_job_role_percentage", coalesce((col("estimated_minus_ascwds")/col(
        "sum_of_estimated_minus_ascwds")), lit(0.0))).drop("estimated_minus_ascwds", "sum_of_estimated_minus_ascwds")
...
def calculate_job_count_breakdown_by_service(master_df):
    # TODO UNIT TEST THIS
    job_role_breakdown_by_service = master_df.selectExpr("primary_service_type AS service_type", "main_job_role AS job_role", "ascwds_num_of_jobs").groupBy(
    ...
    master_df = master_df.join(job_role_breakdown_by_service, (job_role_breakdown_by_service.service_type == master_df.primary_service_type) & (
        job_role_breakdown_by_service.job_role == master_df.main_job_role), 'left').drop('service_type', 'job_role')
...
def determine_worker_record_to_jobs_ratio(master_df):
    master_df = master_df.withColumn(
        "location_jobs_ratio", least(lit(1), col(
            "estimate_job_count_2021") / col("location_worker_records"))
    )
    master_df = master_df.withColumn(
        "location_jobs_to_model", greatest(lit(0), col(
            "estimate_job_count_2021") - col("location_worker_records"))
    )
