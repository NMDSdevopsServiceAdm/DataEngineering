...
target "delta_cqc_providers" {
  context = "."
  dockerfile = "./projects/_01_ingest/cqc_api/fargate/providers/Dockerfile"
  tags = ["${AWS_ACCOUNT_ID}.dkr.ecr.eu-west-2.amazonaws.com/869-dev/cqc:${CIRCLE_BRANCH}"]
  platforms = ["linux/amd64"]
  no-cache = true
}