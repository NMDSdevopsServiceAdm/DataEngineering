resource "aws_sfn_state_machine" "ingest_cqc_api_and_check_for_new_ascwds_state_machine" {
  name     = "${local.workspace_prefix}-Ingest-CQC-API-And-Check-For-New-ASCWDS"
  role_arn = aws_iam_role.step_function_iam_role.arn
  type     = "STANDARD"
  definition = templatefile("step-functions/IngestCQCAPIAndCheckForNewASCWDS-StepFunction.json", {
...
resource "aws_sfn_state_machine" "cqc_api_delta_state_machine" {
  ...
  definition = templatefile("step-functions/IngestCQCAPIDelta-StepFunction.json", {
    ...
    cqc_crawler_name                               = module.cqc_crawler_delta.crawler_name # TODO: point back to main crawler - this one too?
    data_validation_reports_crawler_name           = module.data_validation_reports_crawler.crawler_name
    pipeline_failure_lambda_function_arn           = aws_lambda_function.error_notification_delta_lambda.arn
  })
