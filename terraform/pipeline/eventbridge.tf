    ...
    Statement = [
      {
        ...
        Resource = [
          aws_sfn_state_machine.step_functions_from_files["Ingest-ASCWDS"].arn,
          aws_sfn_state_machine.step_functions_from_files["Ingest-CQC-PIR"].arn,
          aws_sfn_state_machine.step_functions_from_files["Ingest-Capacity-Tracker-Care-Home"].arn,
          aws_sfn_state_machine.step_functions_from_files["Ingest-Capacity-Tracker-Non-Res"].arn,
          aws_sfn_state_machine.step_functions_from_files["Ingest-ONSPD"].arn
        ]
      }
    ]
  })
...
resource "aws_cloudwatch_event_target" "trigger_ingest_ascwds_state_machine" {
  ...
  arn       = aws_sfn_state_machine.step_functions_from_files["Ingest-ASCWDS"].arn
...
resource "aws_cloudwatch_event_target" "trigger_ingest_cqc_pir_state_machine" {
  ...
  arn       = aws_sfn_state_machine.step_functions_from_files["Ingest-CQC-PIR"].arn
  role_arn  = aws_iam_role.start_state_machines.arn
...
resource "aws_cloudwatch_event_target" "trigger_ingest_ons_pd_state_machine" {
  ...
  arn       = aws_sfn_state_machine.step_functions_from_files["Ingest-ONSPD"].arn
  role_arn  = aws_iam_role.start_state_machines.arn
...
resource "aws_cloudwatch_event_target" "trigger_ingest_ct_care_home_state_machine" {
  ...
  arn       = aws_sfn_state_machine.step_functions_from_files["Ingest-Capacity-Tracker-Care-Home"].arn
  role_arn  = aws_iam_role.start_state_machines.arn
...
resource "aws_cloudwatch_event_target" "trigger_ingest_ct_non_res_state_machine" {
  ...
  arn       = aws_sfn_state_machine.step_functions_from_files["Ingest-Capacity-Tracker-Non-Res"].arn
  role_arn  = aws_iam_role.start_state_machines.arn
    ...
    Statement = [
      {
        ...
        Action = ["states:StartExecution"]
        Resource = [
          aws_sfn_state_machine.cqc_and_ascwds_orchestrator_state_machine.arn
        ]
      },
    ]
  })
...
resource "aws_scheduler_schedule" "delta_download_cqc_api_schedule" {
  ...
  target {
    arn      = aws_sfn_state_machine.cqc_and_ascwds_orchestrator_state_machine.arn
