...
def calculate_windowed_column(
    ...
    Returns:
        pl.LazyFrame: A LazyFrame with an additional column containing the specified aggregation function over the given window.

    Raises:
        ValueError: If chosen function does not match 'avg', 'count', 'max', 'min' or 'sum'.
    """
    functions = {
        "avg": pl.col(input_column_name).mean,
        "count": pl.col(input_column_name).count,
        "max": pl.col(input_column_name).max,
        "min": pl.col(input_column_name).min,
        "sum": pl.col(input_column_name).sum,
    }

    if aggregation_function not in functions:
        raise ValueError(
            f"Error: The aggregation function '{aggregation_function}' was not found. "
            "Please use 'avg', 'count', 'max', 'min' or 'sum'."
        )

    expr = functions[aggregation_function]()

    if partition_by_cols:
        expr = expr.over(partition_by_cols)
